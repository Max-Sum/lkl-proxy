...
void lkl_setnonblock(int fd, int *cache)
{
	int flags;
	if (!cache || *cache == -1) {
	 	flags = lkl_call(__lkl__NR_fcntl, 3, fd, LKL_F_GETFL, 0);
		if (cache)
			*cache = flags;
	} else
		flags = *cache;
	lkl_call(__lkl__NR_fcntl, 3, fd, LKL_F_SETFL, flags|LKL_O_NONBLOCK);
}
...
int lkl_epoll_add(int efd, int fd, int revents, void *conn)
{
	...
	return lkl_sys_epoll_ctl(efd, EPOLL_CTL_ADD, fd, &ev);
}

int lkl_epoll_del(int efd, int fd)
{
	num_events--;
	assert(num_events >= 0);
	return lkl_epoll_ctl(efd, EPOLL_CTL_DEL, fd, (void *)1L);
}
...
void lkl_new_request(int efd, int lfd, int *cache, time_t now)
{
	int newsk = lkl_sys_accept(lfd, NULL, NULL);
	if (newsk < 0) {
		perror("accept");
		return;
	}
	// xxx log
	lkl_setnonblock(newsk, cache);
...
int lkl_listen_socket(int efd, char *lname, char *port)
{
	struct addrinfo *laddr = resolve(lname, port, AI_PASSIVE);
	
	int lfd = lkl_sys_socket(laddr->ai_family, LKL_SOCK_STREAM, 0);
	if (lfd < 0) 
		err("socket");
	int opt = 1;
	if (lkl_sys_setsockopt(lfd, LKL_SOL_SOCKET, LKL_SO_REUSEADDR, &opt, sizeof(int)) < 0) 
		err("SO_REUSEADDR");
	if (lkl_sys_bind(lfd, laddr->ai_addr, laddr->ai_addrlen) < 0) 
		err("bind");
	if (lkl_sys_listen(lfd, PROXY_LISTEN_BACKLOG) < 0) 
		err("listen");
	lkl_setnonblock(lfd, NULL);
	freeaddrinfo(laddr);
...
int main(int ac, char **av)
{
	...
	int efd = lkl_sys_epoll_create(10);
	if (efd < 0) 
		err("epoll_create");

	int lfd = lkl_listen_socket(efd, av[4] ? av[4] : "0.0.0.0", av[1]);

	int cache_in = -1, cache_out = -1;	
	int timeo = -1;

	for (;;) {
		int nfds = lkl_sys_epoll_wait(efd, events, num_events, timeo);
		if (nfds < 0) {
			perror("epoll");
			continue;
		}
